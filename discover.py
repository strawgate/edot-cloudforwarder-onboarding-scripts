...
def generate_cloudformation_command(
    ...
    """Generate a CloudFormation create-stack command."""
    return f"""aws cloudformation create-stack \\
  --stack-name {stack_name} \\
  --template-url {CLOUDFORMATION_TEMPLATE_URL} \\
  --capabilities CAPABILITY_NAMED_IAM \\
  --region {region} \\
  --parameters \\
    ParameterKey=OTLPEndpoint,ParameterValue={otlp_endpoint} \\
    ParameterKey=ElasticAPIKey,ParameterValue={api_key} \\
    ParameterKey=EdotCloudForwarderS3LogsType,ParameterValue={log_type} \\
    ParameterKey=S3BucketARN,ParameterValue={bucket_arn}"""
...
def generate_deployment_commands(
    ...
    Returns list of tuples: (display_name, bucket_arn, log_type, command)
    ...
    for (bucket_arn, log_type, region), sources in buckets_by_type.items():
        if log_type == "vpc_flow_logs":
            vpc_counter += 1
            stack_name = f"edot-cf-vpc-flow-{vpc_counter}"
            display_name = "VPC Flow Logs"
        else:
            elb_counter += 1
            stack_name = f"edot-cf-elb-access-{elb_counter}"
...
def execute_deployment(command: str) -> tuple[bool, str]:
